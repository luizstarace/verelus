...
src/
  lib/
    ...
    types/
      atalaia.ts      — Interfaces TypeScript do dominio
  ...
  app/
    ...
    health/atalaia/   — GET health check (Supabase + Claude)
    cron/health/       — Cron cada 5min com alerta email

    atalaia/          — Landing page publica
    dashboard/atalaia/
...
supabase/migrations/
  004_atalaia.sql     — 5 tabelas + 3 RPCs + RLS
```

## Tabelas (Supabase)

- `atalaia_businesses` — Dados do negocio, ai_context, widget_config, status
- `atalaia_conversations` — Conversas (widget/whatsapp/voice), status, satisfaction
- `atalaia_messages` — Mensagens (customer/assistant/human), tokens, audio_url
- `atalaia_usage` — Uso mensal: text_messages, voice_seconds, tokens_total
- `atalaia_logs` — Observabilidade: endpoint, latency, errors (sem RLS)
...
# Atalaia-especificas
STRIPE_PRICE_ATALAIA_STARTER=price_1TOgy42MFMsgV0iG4weI9yYy
STRIPE_PRICE_ATALAIA_PRO=price_1TOgy42MFMsgV0iGPMxHJiHr
STRIPE_PRICE_ATALAIA_BUSINESS=price_1TOgy52MFMsgV0iGwxgrl0gI
...
- [Migration SQL](sql/004_atalaia.sql)
