...
**Data:** 2026-04-18
**Status:** Aprovado
**Produto:** Atalaia (produto dentro da plataforma Verelus)

---

## 1. Visao Geral

Atalaia e um atendente IA para PMEs brasileiras. Responde clientes via WhatsApp e widget de chat no site, 24/7, com voz opcional (ElevenLabs). O dono do negocio cadastra servicos, precos e horarios; a IA aprende e atende como se fosse ele.

**Marca:** Verelus e a marca guarda-chuva (generica, multi-produto). Atalaia e o primeiro produto. Cada produto tem identidade propria mas herda a paleta da Verelus.
...
verelus.com/atalaia                → Landing page do Atalaia
verelus.com/atalaia#pricing        → Ancora na landing (scroll pra pricing)
verelus.com/login                   → Auth compartilhado (ja existe)
verelus.com/dashboard/atalaia      → Dashboard do cliente
  /dashboard/atalaia/setup         → Onboarding wizard
  /dashboard/atalaia/inbox         → Conversas do atendente
  /dashboard/atalaia/settings      → Configuracoes
  /dashboard/atalaia/billing       → Plano e uso
```
...
-- DELETE FROM atalaia_logs WHERE created_at < now() - interval '30 days';
```
...
- atalaia_logs nao tem RLS — tabela interna de observabilidade
- Reutiliza tabela `subscriptions` existente com `product='atalaia'`
...
```
Dashboard /atalaia/setup (wizard com checklist — 5 passos)

  Passo 1: Dados do negocio (nome, categoria, telefone, endereco)
  → PATCH /api/atalaia/business {step: 1, data: {...}}
  → Salva parcial em atalaia_businesses
  → onboarding_step = 2

  Passo 2: Servicos + Horarios + FAQ
  → PATCH /api/atalaia/business {step: 2, data: {...}}
  ...
  → POST /api/atalaia/chat {preview: true}
...
POST /api/atalaia/chat
  ...
  → Ao finalizar stream: salva resposta completa em atalaia_messages
    (garante persistencia mesmo se conexao cair mid-stream)
  → Salva mensagem do cliente em atalaia_messages

  Pos-resposta:
  → Incrementa message_count na conversation
  → Incrementa text_messages + tokens em atalaia_usage
  ...
  → Loga em atalaia_logs (latency, tokens, status)
...
Cliente manda msg no WhatsApp
  ...
  → n8n chama POST /api/atalaia/chat {channel: 'whatsapp'}
  → Recebe resposta
...
POST /api/atalaia/voice
  ...
  → Incrementa voice_seconds em atalaia_usage
...
- Excedente: calculado no fim do mes via atalaia_usage, invoice item adicional no Stripe
...
e carregada via `GET /api/atalaia/widget/[id]/config` no init (JSON leve, ~1KB).
...
- **Powered by Atalaia:** com referral tracking (?ref=widget_BUSINESS_ID)
...
├── Produto Destaque: card Atalaia (grande, CTA "Conhecer →")
...
├── Comparacao: Atendente humano vs Atalaia (tabela)
...
### Conversas (/dashboard/atalaia/inbox)
...
### Configuracoes (/dashboard/atalaia/settings)
...
### Plano & Uso (/dashboard/atalaia/billing)
...
```
POST   /api/atalaia/setup                    → cria business + gera ai_context
GET    /api/atalaia/business                 → dados do negocio
PATCH  /api/atalaia/business                 → atualiza + regenera ai_context
POST   /api/atalaia/chat                     → chat (SSE stream)
POST   /api/atalaia/voice                    → text-to-speech
GET    /api/atalaia/conversations            → lista com filtros
GET    /api/atalaia/conversations/[id]       → detalhe + mensagens
POST   /api/atalaia/conversations/[id]/reply → resposta humana
PATCH  /api/atalaia/conversations/[id]/status → fechar/devolver pra IA
GET    /api/atalaia/widget.js                 → script generico (cacheable CDN)
GET    /api/atalaia/widget/[id]/config       → config do business (JSON)
POST   /api/atalaia/widget/lead              → captura pre-chat
POST   /api/atalaia/whatsapp/connect         → gera QR code
GET    /api/atalaia/whatsapp/status          → status conexao
POST   /api/atalaia/whatsapp/webhook         → recebe msgs Evolution API
GET    /api/atalaia/usage                    → uso periodo atual
POST   /api/atalaia/checkout                 → sessao Stripe
GET    /api/health/atalaia                   → health check
```

### Observabilidade

- `GET /api/health/atalaia` checa: Supabase, Claude API, ElevenLabs, Evolution API
- n8n cron (10min): chama health endpoint, email se falha
- Logs estruturados em `atalaia_logs`: business_id, endpoint, channel, tokens, latency_ms, status_code
