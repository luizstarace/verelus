# Attendly Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build Attendly — an AI customer service agent for Brazilian SMBs — as the first product inside the Verelus multi-product platform.

**Architecture:** Monolith expansion of the existing Next.js 14 Verelus app. Attendly lives under `/attendly` (landing) and `/dashboard/attendly` (app). Shares auth, Stripe, and Supabase with the existing codebase. Chat uses Claude Haiku 4.5 via SSE streaming. Widget is a vanilla JS embeddable with Shadow DOM. WhatsApp integration via n8n + Evolution API.
...
```
# Database
supabase/migrations/004_attendly.sql

# Types
src/lib/types/attendly.ts

# API Routes
src/app/api/attendly/setup/route.ts
src/app/api/attendly/business/route.ts
src/app/api/attendly/chat/route.ts
src/app/api/attendly/voice/route.ts
src/app/api/attendly/conversations/route.ts
src/app/api/attendly/conversations/[id]/route.ts
src/app/api/attendly/conversations/[id]/reply/route.ts
src/app/api/attendly/conversations/[id]/status/route.ts
src/app/api/attendly/widget/lead/route.ts
src/app/api/attendly/widget/[id]/config/route.ts
src/app/api/attendly/whatsapp/connect/route.ts
src/app/api/attendly/whatsapp/status/route.ts
src/app/api/attendly/whatsapp/webhook/route.ts
src/app/api/attendly/usage/route.ts
src/app/api/attendly/checkout/route.ts
src/app/api/health/attendly/route.ts

# Attendly helpers
src/lib/attendly/ai-context.ts
src/lib/attendly/chat.ts
src/lib/attendly/usage.ts
src/lib/attendly/transfer.ts
src/lib/attendly/plans.ts
src/lib/attendly/notifications.ts

# Pages
src/app/page.tsx                              (modify — homepage rebrand)
src/app/attendly/page.tsx                     (landing page)
src/app/dashboard/attendly/page.tsx           (overview)
src/app/dashboard/attendly/setup/page.tsx
src/app/dashboard/attendly/setup/SetupWizard.tsx
src/app/dashboard/attendly/inbox/page.tsx
src/app/dashboard/attendly/inbox/InboxView.tsx
src/app/dashboard/attendly/inbox/ConversationDetail.tsx
src/app/dashboard/attendly/settings/page.tsx
src/app/dashboard/attendly/settings/SettingsView.tsx
src/app/dashboard/attendly/billing/page.tsx
src/app/dashboard/attendly/billing/BillingView.tsx
src/app/dashboard/attendly/OverviewDashboard.tsx

# Widget
public/widget.js

# Tests
src/__tests__/api/attendly/setup.test.ts
src/__tests__/api/attendly/chat.test.ts
src/__tests__/api/attendly/business.test.ts
src/__tests__/api/attendly/conversations.test.ts
src/__tests__/api/attendly/usage.test.ts
src/__tests__/lib/attendly/ai-context.test.ts
src/__tests__/lib/attendly/chat.test.ts
src/__tests__/lib/attendly/transfer.test.ts
src/__tests__/lib/attendly/plans.test.ts
```
...
src/app/dashboard/layout.tsx                  (add Attendly sidebar items)
src/lib/translations.ts                       (add Attendly translations)
src/lib/use-user-tier.ts                      (extend for Attendly plans)
```
...
**Files:**
- Create: `supabase/migrations/004_attendly.sql`
- Create: `src/lib/types/attendly.ts`
- Create: `src/lib/attendly/plans.ts`
- Modify: `tailwind.config.ts`
- Modify: `src/app/layout.tsx`

- [ ] **Step 1: Write Attendly types**

```typescript
// src/lib/types/attendly.ts

export interface AttendlyBusiness {
  id: string;
  user_id: string;
  name: string;
...
export interface AttendlyService {
  name: string;
  price_cents: number;
  duration_min: number;
  description: string;
...
export interface AttendlyConversation {
  id: string;
  business_id: string;
...
export interface AttendlyMessage {
  id: string;
...
export interface AttendlyUsage {
  id: string;
  business_id: string;
  period: string;
...
export type AttendlyPlan = 'starter' | 'pro' | 'business';

export interface AttendlyPlanLimits {
  text_messages: number;
  voice_seconds: number;
...
Run: `cd /Users/luizsfap/Desktop/Claude\ CODE\ Projects/verelus && npx vitest run src/__tests__/lib/attendly/plans.test.ts`
...
```typescript
// Replace the existing brand colors with the new Attendly palette
colors: {
  brand: {
...
```bash
git add supabase/migrations/004_attendly.sql src/lib/types/attendly.ts src/lib/attendly/plans.ts src/__tests__/lib/attendly/plans.test.ts tailwind.config.ts src/app/layout.tsx
git commit -m "feat: attendly foundation — schema, types, plans, tailwind rebrand"
```
...
**Files:**
- Create: `src/lib/attendly/ai-context.ts`
- Create: `src/__tests__/lib/attendly/ai-context.test.ts`
...
Run: `npx vitest run src/__tests__/lib/attendly/ai-context.test.ts`
...
```bash
git add src/lib/attendly/ai-context.ts src/__tests__/lib/attendly/ai-context.test.ts
git commit -m "feat: ai-context generator for Attendly businesses"
```
...
**Files:**
- Create: `src/lib/attendly/transfer.ts`
- Create: `src/lib/attendly/chat.ts`
- Create: `src/__tests__/lib/attendly/transfer.test.ts`
- Create: `src/__tests__/lib/attendly/chat.test.ts`
...
Run: `npx vitest run src/__tests__/lib/attendly/transfer.test.ts`
Expected: FAIL
...
Run: `npx vitest run src/__tests__/lib/attendly/chat.test.ts`
Expected: All PASS

- [ ] **Step 8: Commit**

```bash
git add src/lib/attendly/transfer.ts src/lib/attendly/chat.ts src/__tests__/lib/attendly/transfer.test.ts src/__tests__/lib/attendly/chat.test.ts
git commit -m "feat: transfer detection and chat helpers for Attendly"
```
...
**Files:**
- Create: `src/app/api/attendly/setup/route.ts`
- Create: `src/app/api/attendly/business/route.ts`

- [ ] **Step 1: Implement POST /api/attendly/setup**
...
- [ ] **Step 2: Implement GET & PATCH /api/attendly/business**
...
curl -X POST http://localhost:3000/api/attendly/setup \
...
```bash
git add src/app/api/attendly/setup/route.ts src/app/api/attendly/business/route.ts
git commit -m "feat: business setup and CRUD API for Attendly"
```
...
**Files:**
- Create: `src/app/api/attendly/chat/route.ts`
- Create: `src/lib/attendly/usage.ts`
...
- [ ] **Step 2: Implement POST /api/attendly/chat**
...
Append to `supabase/migrations/004_attendly.sql`:
...
```bash
curl -N http://localhost:3000/api/attendly/chat \
  -H "Content-Type: application/json" \
...
```bash
git add src/app/api/attendly/chat/route.ts src/lib/attendly/usage.ts supabase/migrations/004_attendly.sql
...
**Files:**
- Create: `src/app/api/attendly/conversations/route.ts`
- Create: `src/app/api/attendly/conversations/[id]/route.ts`
- Create: `src/app/api/attendly/conversations/[id]/reply/route.ts`
- Create: `src/app/api/attendly/conversations/[id]/status/route.ts`
- Create: `src/app/api/attendly/usage/route.ts`
- Create: `src/app/api/health/attendly/route.ts`

- [ ] **Step 1: Implement GET /api/attendly/conversations**
...
- [ ] **Step 2: Implement GET /api/attendly/conversations/[id]**
...
- [ ] **Step 3: Implement POST /api/attendly/conversations/[id]/reply**
...
- [ ] **Step 4: Implement PATCH /api/attendly/conversations/[id]/status**
...
- [ ] **Step 5: Implement GET /api/attendly/usage**
...
import { NextResponse } from 'next/server';
import { requireUser, errorResponse } from '@/lib/api-auth';
import { getCurrentPeriod } from '@/lib/attendly/chat';
...
- [ ] **Step 6: Implement GET /api/health/attendly**
...
```bash
git add src/app/api/attendly/conversations/ src/app/api/attendly/usage/route.ts src/app/api/health/attendly/route.ts
...
- Product card: Attendly with CTA → /attendly
...
Add Attendly sections to the translations object:
...
Verify: New homepage with light background, blue+orange palette, Attendly card, email capture form.
...
## Task 8: Attendly Landing Page

**Files:**
- Create: `src/app/attendly/page.tsx`

- [ ] **Step 1: Create the Attendly landing page**

Build `src/app/attendly/page.tsx` following spec section 10. Server component. Sections:
- Hero with CTA
...
Run: `npm run dev`
Open: `http://localhost:3000/attendly`
Verify: Full landing page renders with all sections, responsive on mobile.

- [ ] **Step 3: Commit**

```bash
git add src/app/attendly/page.tsx
git commit -m "feat: Attendly landing page with pricing and comparison"
```

---

## Task 9: Dashboard Layout + Attendly Overview

**Files:**
- Modify: `src/app/dashboard/layout.tsx`
- Create: `src/app/dashboard/attendly/page.tsx`
- Create: `src/app/dashboard/attendly/OverviewDashboard.tsx`

- [ ] **Step 1: Read current dashboard layout**

Read `src/app/dashboard/layout.tsx` to understand sidebar structure.

- [ ] **Step 2: Add Attendly items to sidebar**

Modify `src/app/dashboard/layout.tsx`:
- Add sidebar items: Visão Geral, Conversas, Configurações, Plano & Uso
- Routes: `/dashboard/attendly`, `/dashboard/attendly/inbox`, `/dashboard/attendly/settings`, `/dashboard/attendly/billing`
...
```typescript
// src/app/dashboard/attendly/page.tsx
export const runtime = 'edge';
  ...
  if (!business) {
    return (
      <div className="p-8 text-center">
        ...
        <a href="/dashboard/attendly/setup" className="bg-brand-cta text-brand-text font-bold px-6 py-3 rounded-lg hover:brightness-110">
    ...
    <div className="p-8 space-y-6">
      ...
      {stats && stats.usage_percentage >= 80 && (
        <div className="bg-brand-cta/10 border border-brand-cta/30 rounded-lg p-4 text-brand-cta text-sm">
          ⚠️ Uso em {stats.usage_percentage}% do limite. <a href="/dashboard/attendly/billing" className="underline font-bold">Ver plano</a>
        </div>
      )}
...
Open: `http://localhost:3000/dashboard/attendly`
Verify: Overview page loads, shows "Começar configuração" if no business, or stats if business exists.

- [ ] **Step 6: Commit**

```bash
git add src/app/dashboard/layout.tsx src/app/dashboard/attendly/page.tsx src/app/dashboard/attendly/OverviewDashboard.tsx
git commit -m "feat: dashboard layout + Attendly overview page"
```
...
**Files:**
- Create: `src/app/dashboard/attendly/setup/page.tsx`
- Create: `src/app/dashboard/attendly/setup/SetupWizard.tsx`
...
Key behaviors:
- Each step saves via `PATCH /api/attendly/business` or `POST /api/attendly/setup` (step 1)
...
Open: `http://localhost:3000/dashboard/attendly/setup`
...
- Completion redirects to `/dashboard/attendly`

- [ ] **Step 4: Commit**

```bash
git add src/app/dashboard/attendly/setup/
...
**Files:**
- Create: `src/app/dashboard/attendly/inbox/page.tsx`
- Create: `src/app/dashboard/attendly/inbox/InboxView.tsx`
- Create: `src/app/dashboard/attendly/inbox/ConversationDetail.tsx`
...
Open: `http://localhost:3000/dashboard/attendly/inbox`
Verify: Conversations list loads, clicking opens detail, human reply works.

- [ ] **Step 5: Commit**

```bash
git add src/app/dashboard/attendly/inbox/
...
**Files:**
- Create: `src/app/dashboard/attendly/settings/page.tsx`
- Create: `src/app/dashboard/attendly/settings/SettingsView.tsx`
- Create: `src/app/dashboard/attendly/billing/page.tsx`
- Create: `src/app/dashboard/attendly/billing/BillingView.tsx`
...
All edits go through `PATCH /api/attendly/business`.
...
- "Fazer upgrade" button → links to `/attendly#pricing` or Stripe checkout
- Invoice history → Stripe Customer Portal link

Data from `GET /api/attendly/usage`.
...
```bash
git add src/app/dashboard/attendly/settings/ src/app/dashboard/attendly/billing/
...
- Create: `src/app/api/attendly/widget/[id]/config/route.ts`
- Create: `src/app/api/attendly/widget/lead/route.ts`
...
- Fetches config from `/api/attendly/widget/[id]/config`
- Creates Shadow DOM container
- Pre-chat form: name (required) + phone (optional) → calls `/api/attendly/widget/lead`
- Chat interface with bubbles
- Sends messages to `/api/attendly/chat` via fetch
...
```bash
git add public/widget.js src/app/api/attendly/widget/
...
**Files:**
- Create: `src/app/api/attendly/voice/route.ts`

- [ ] **Step 1: Implement POST /api/attendly/voice**
...
Via Supabase Dashboard: create bucket `attendly-audio` with public access.

Or via SQL:
```sql
INSERT INTO storage.buckets (id, name, public) VALUES ('attendly-audio', 'attendly-audio', true);
```

- [ ] **Step 4: Commit**

```bash
git add src/app/api/attendly/voice/route.ts supabase/migrations/004_attendly.sql
...
**Files:**
- Create: `src/app/api/attendly/checkout/route.ts`
- Modify: `src/app/api/webhook/stripe/route.ts`

- [ ] **Step 1: Implement POST /api/attendly/checkout**
...
const PRICE_MAP: Record<string, string> = {
  ...
  attendly_starter_annual: process.env.STRIPE_PRICE_ATTENDLY_STARTER_ANNUAL || '',
  attendly_pro_annual: process.env.STRIPE_PRICE_ATTENDLY_PRO_ANNUAL || '',
  attendly_business_annual: process.env.STRIPE_PRICE_ATTENDLY_BUSINESS_ANNUAL || '',
};
...
- [ ] **Step 2: Update Stripe webhook to handle Attendly subscriptions**
...
- If `status === 'active'`, update `attendly_businesses.status = 'active'`

On subscription deleted:
- Update subscription status to 'canceled'
- Update `attendly_businesses.status = 'paused'`

- [ ] **Step 3: Commit**

```bash
git add src/app/api/attendly/checkout/route.ts src/app/api/webhook/stripe/route.ts
git commit -m "feat: Stripe checkout and webhook for Attendly subscriptions"
```
...
**Files:**
- Create: `src/app/api/attendly/whatsapp/connect/route.ts`
- Create: `src/app/api/attendly/whatsapp/status/route.ts`
- Create: `src/app/api/attendly/whatsapp/webhook/route.ts`
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Extract business_id from instance name (attendly_<uuid>)
...
```bash
git add src/app/api/attendly/whatsapp/
...
**Files:**
- Create: `src/lib/attendly/notifications.ts`
...
export function buildTransferEmail(businessName: string, customerName: string, conversationUrl: string): NotifyOwnerParams {
  return {
    to: '', // filled by caller
    subject: `[Attendly] ${customerName || 'Cliente'} precisa da sua atenção`,
    html: `
...
export function buildUsageAlertEmail(businessName: string, percentage: number, billingUrl: string): NotifyOwnerParams {
  return {
    to: '',
    subject: `[Attendly] Uso em ${percentage}% do limite`,
    html: `
      <h2>Alerta de uso</h2>
...
export function buildTrialExpiryEmail(businessName: string, daysLeft: number, checkoutUrl: string): NotifyOwnerParams {
  return {
    to: '',
    subject: `[Attendly] Seu trial acaba em ${daysLeft} dia${daysLeft > 1 ? 's' : ''}`,
    html: `
      <h2>Trial expirando</h2>
      <p>O trial de <strong>${businessName}</strong> acaba em ${daysLeft} dia${daysLeft > 1 ? 's' : ''}.</p>
      <p>Adicione seu cartão para continuar usando o Attendly sem interrupção.</p>
...
In `src/app/api/attendly/chat/route.ts`, after detecting transfer:

```typescript
// After setting status to 'human_needed', send notification
import { notifyOwnerEmail, buildTransferEmail } from '@/lib/attendly/notifications';
...
In `src/lib/attendly/usage.ts`, after incrementing usage, check if >80% and notify.

- [ ] **Step 4: Commit**

```bash
git add src/lib/attendly/notifications.ts src/app/api/attendly/chat/route.ts src/lib/attendly/usage.ts
...
**Files:**
- Modify: `src/middleware.ts` (ensure Attendly routes don't interfere with auth)
...
- `/api/attendly/chat` (public — called by widget without auth)
- `/api/attendly/widget/*` (public — config and lead capture)
- `/api/attendly/whatsapp/webhook` (public — called by n8n)
- `/api/health/attendly` (public — called by monitoring)
...
3. Click "Conhecer" → lands on `/attendly` — verify landing page
4. Click "Testar 7 dias grátis" → redirect to login
5. Log in → redirected to `/dashboard/attendly`
6. Click "Começar configuração" → `/dashboard/attendly/setup`
...
```bash
git add .
git commit -m "feat: Attendly MVP complete — AI customer service for SMBs"
```
...
| 8 | Attendly Landing Page | attendly/page.tsx |
| 9 | Dashboard Layout + Overview | dashboard/attendly/ |
