...
// Shared landing-page shell consumed by /attendly/page.tsx and the vertical
// pages under /attendly/{slug}/page.tsx. Only the H1, hero subtitle and FAQ
// vary per vertical; everything else (nav, comparison, pricing, etc.) is
// identical across pages.

const SITE_BASE = 'https://verelus.com';

export default function AttendlyLanding({ config }: { config: VerticalConfig }) {
  const fullUrl = `${SITE_BASE}${config.canonicalPath}`;

  // JSON-LD: SoftwareApplication (parameterised URL), BreadcrumbList, FAQPage.
  const softwareLd = {
  ...
  const breadcrumbLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement:
      config.slug === 'attendly'
        ? [
            { '@type': 'ListItem', position: 1, name: 'Verelus', item: SITE_BASE },
            { '@type': 'ListItem', position: 2, name: 'Attendly', item: fullUrl },
          ]
        : [
            { '@type': 'ListItem', position: 1, name: 'Verelus', item: SITE_BASE },
            { '@type': 'ListItem', position: 2, name: 'Attendly', item: `${SITE_BASE}/attendly` },
          ...
          <div className="flex items-center gap-6">
            <Link
              ...
              className={`text-sm font-semibold pb-0.5 ${
                config.slug === 'attendly'
                  ? 'text-brand-trust border-b-2 border-brand-trust'
                  : 'text-brand-muted hover:text-brand-trust transition-colors'
              }`}
            >
              Attendly
            </Link>
