...
export default function SetupWizard() {
  ...
  async function handleStep1Next() {
    ...
    let data;
    if (businessId) {
      data = await apiCall('/api/attendly/business', 'PATCH', { ...payload, onboarding_step: 2 });
    } else {
      data = await apiCall('/api/attendly/setup', 'POST', payload);
    }
    if (data) {
  ...
  async function handleSendChat() {
    ...
    try {
      const res = await fetch('/api/attendly/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  async function handleStep4Next() {
    const data = await apiCall('/api/attendly/business', 'PATCH', { onboarding_step: 5 });
    if (data) setStep(5);
  }
    ...
    return (
      <div className="flex items-center justify-between pt-6 border-t border-brand-border">
        ...
        <div className="flex gap-3">
          ...
          {isLast && (
            <button type="button" onClick={() => router.push('/dashboard/attendly')} className={btnSecondary} disabled={loading}>
              Pular
            </button>
          )}
