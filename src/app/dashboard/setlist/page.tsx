  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch("/api/ai/generate", {
        ...
        body: JSON.stringify({ type: "setlist" }),
      });
    ...
    <div className="min-h-screen bg-bg-primary">
      <header className="border-b border-white/10 bg-bg-secondary/50 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center gap-4">
          ...
          <h1 className="font-display text-xl text-white tracking-wider">GEST\u00c3O DE SETLISTS</h1>
        </div>
      </header>
          ...
          <div className="lg:col-span-3">
            <div className="bg-bg-surface border border-white/10 rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-white">Setlist Inteligente</h2>
                <button
              ...
              {content ? (
                ...
                <div className="text-center py-16 text-gray-400">
                  <p className="text-4xl mb-4">\u{1F3B6}</p>
                  <p className="text-sm">Gere setlists inteligentes baseadas em dados</p>
                </div>
              )}
            </div>
          </div>
