-- Attendly infra hardening
-- 1. stripe_events_processed: idempotency for Stripe webhook retries
-- 2. trial_ends_at on attendly_businesses: enforce "7 dias grátis sem cartão" promise
-- 3. attendly-audio storage bucket (private; voice endpoint uses signed URLs)
...
ALTER TABLE attendly_businesses
...
VALUES ('attendly-audio', 'attendly-audio', false, 5242880, ARRAY['audio/mpeg', 'audio/mp3'])
